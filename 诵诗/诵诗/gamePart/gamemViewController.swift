...
//        }
        ...
        var stmt = singleViewController.stmt
        //编译
        if sqlite3_prepare_v2(singleViewController.db, query.cString(using: .utf8), -1, &stmt, nil) != SQLITE_OK {
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            if let temp = UnsafePointer(sqlite3_column_text(stmt, Int32(0))){
...
//            levelNum = level
        ...
        //编译
        if sqlite3_prepare_v2(singleViewController.db, sql.cString(using: .utf8), -1, &stmt, nil) != SQLITE_OK {
            sqlite3_finalize(stmt)
            if (sqlite3_errmsg(singleViewController.db)) != nil {
